...
const PythonPlayground: React.FC = () => {
    ...
    const initPyodide = async () => {
      try {
        if (window.loadPyodide && !pyodide) {
          ...
          setPyodide(py);
          setOutput(prev => [...prev, "Ready! Click 'Run' to execute code."]);
        }
      } catch (err) {
  ...
  const resetCode = () => {
    if (window.confirm("Reset code to original example?")) {
  ...
  return (
    <div className="flex flex-col h-[600px] border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden shadow-xl">
      {/* Toolbar */}
      <div className="bg-slate-100 dark:bg-slate-800 p-2 flex gap-2 border-b border-slate-200 dark:border-slate-700">
        <button
          onClick={runCode}
        ...
        <button
          onClick={resetCode}
          className="px-4 py-1.5 rounded font-medium text-slate-700 dark:text-slate-300 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-sm transition"
        >
          Reset
        </button>
      ...
      <div className="flex-1 flex flex-col md:flex-row">
        ...
        {/* Output */}
        <div className="w-full md:w-1/3 bg-slate-900 text-slate-300 font-mono text-xs md:text-sm p-4 overflow-y-auto border-t md:border-t-0 md:border-l border-slate-700" ref={outputRef}>
          <div className="uppercase tracking-wider text-slate-500 mb-2 text-xs font-bold">Terminal Output</div>
